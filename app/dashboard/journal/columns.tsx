...
export function getColumns(
  ...
  accessorFn: (row: TradeWithPair) =>
    row.customName ?? row.pair?.pair?.symbol ?? "-",
  header: () => <div className="text-center px-3 py-2">Simbol</div>,
  cell: ({ getValue }) => (
    <div className="text-left px-3 py-2">{getValue() as string}</div>
  ),
},
        ...
        return (
          <div className="flex justify-center px-2 py-2">
            <TradeDetailDialog
              ...
              userPsychologies={userPsychologies}
            />
          </div>
        );
      },
